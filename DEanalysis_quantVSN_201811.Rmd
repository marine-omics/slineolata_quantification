...
anno_data <- read_tsv(file = "LFQmean_annotatedtable_201705.txt")
#only important data
anno_data <- anno_data[c(1,423,427,432,433)]
```

```{r}
# This is a tibble (not a data.frame) and it allows column names with spaces.  
# To make things behave like a data.frame I do this 
#
...
#remove rows with NAs in more than 50% of the columns 
#filtered_data <- filtered_data[-which(rowMeans(is.na(filtered_data)) > 0.90),]
```
...
Separation of quantiation values
Dont change row order
```{r}
...
#normalisation using VSN - no log needed, works on raw values
...
mds_data <- data.frame(x=mds_data$x,y=mds_data$y,sample_data[names(mds_data$x),])
plot = ggplot(mds_data,aes(x=x,y=y)) + geom_point(aes(color=Tissue, shape=Tissue), size=3) 

#to output plot
#png(file="MDS_vsn.png", width = 698, height = 550)
#plot + theme_pubr(base_size = 18, legend = c("right"))
#dev.off()
```

Code to extract results and make heatmaps using just count data (normalized log2) comparisons
```{r}
#comparisons go here for countdata
...
#to output graph
#png(file="HM_abund.png", width = 698, height = 550)
#HMab <- draw(HMab, annotation_legend_side = "right")
#dev.off()
```

Code to extract results for comparisons with avg as 'control' using logFC

```{r NEW HM WITH SLIME COMP}
...
#old#slimesubfilter3 <- subset(anno_merge, anno_merge$SlimevsAv > 0 & anno_merge$VMMvsAv < 0 & anno_merge$DMMvsAv < 0 & anno_merge$VMGvsAv > -1)
...
#write filteredtable
#write.table(slimesubfilter3, file = 'DEslime.txt',row.names = FALSE, col.names = TRUE ,quote = FALSE, sep = '\t')
...
#as pdf
#pdf("HM_vsn.pdf", width = 10, height = 8)
#dev.off()
...
geneID2GO <- readMappings(file = "goblastprots.map")
...
printGraph(GOdata, resultFis, firstSigNodes = numsignif, fn.prefix = "tGO5_vsn", useInfo = "def", pdfSW = TRUE)


AnnotatedGenes = lapply(allRes$GO.ID, function(x) as.character(unlist(genesInTerm(object = GOdata, whichGO = x)))) # list containg genes annotated to significant GO terms
